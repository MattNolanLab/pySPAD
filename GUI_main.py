...
class SPAD_GUI(QtGui.QWidget):
    ...
    def __init__(self, parent = None):
        ...
        #live imaging groupbox 
        
        self.liveImaging_groupbox = QtGui.QGroupBox('Live imaging')
        self.aggregation_label = QtGui.QLabel('Aggregation(frames): ')
        self.aggregation_combo = QtGui.QComboBox()
        self.aggregation_combo.setFixedWidth(80)
        self.combo_list = ['1','2','4','8','16','32','64','128','256','512','1024']
        self.aggregation_combo.addItems(self.combo_list)
        self.captureBackground_button = QtGui.QPushButton('Capture Background')
        self.captureBackground_button.setFixedWidth(200)
        self.captureBackground_button.setEnabled(False)
        
        
        
        
        
        self.liveImaginggroup_layout = QtGui.QHBoxLayout()
        self.liveImaginggroup_layout.addWidget(self.aggregation_label)
        self.liveImaginggroup_layout.addWidget(self.aggregation_combo)
        self.liveImaginggroup_layout.addWidget(self.captureBackground_button)
        self.liveImaging_groupbox.setLayout(self.liveImaginggroup_layout)
        ...
        self.horizontal_layout_4.addWidget(self.liveImaging_groupbox)
    ...
    def start(self):
        ...
        self.captureBackground_button.setEnabled(True)
        self.record_button.setEnabled(True)
        self.stop_button.setEnabled(True)
        self.status_text.setText('Running')
        
        aggregate = int(self.aggregation_combo.currentText())
        self.captureBackground_button.clicked.connect(self.save_background)
    ...
    def save_background(self):
        ...
        n = int(self.aggregation_combo.currentText())
        new_data = np.asarray(list(self.sensor.GetLiveData(n)))
        converted_data = new_data.reshape((-1,4))
        sum_bits = converted_data.sum(axis=1)
        self.background_image = sum_bits.reshape(240,320)
    ...
    def update(self):
        ...
        if self.background_on==True:
            ...
            back_image = self.background_image
            ...
            new_data = np.asarray(list(self.sensor.GetLiveData(aggregate)))
            converted_data = new_data.reshape((-1,4))
            sum_bits = converted_data.sum(axis=1)
            image = image + (sum_bits.reshape(240,320)-back_image)
            self.green_signal = self.ROI_region_green.getArrayRegion(sum_bits.reshape(240,320), self.sensor_image).sum()
            self.red_signal = self.ROI_region_red.getArrayRegion(sum_bits.reshape(240,320), self.sensor_image).sum()
            
             #self.data2[:-1] = self.data2[1:]                   
            self.data1[-1] = self.red_signal
    ...
    #new_data = np.unpackbits(self.sensor.GetLiveData()).sum() 
            ...
            converted_data = new_data.reshape((-1,4))
            sum_bits = converted_data.sum(axis=1)
            image = image + sum_bits.reshape(240,320)
            self.green_signal = self.ROI_region_green.getArrayRegion(sum_bits.reshape(240,320), self.sensor_image).sum()
            self.red_signal = self.ROI_region_red.getArrayRegion(sum_bits.reshape(240,320), self.sensor_image).sum()
        ...
        # colors = [(25,25,112), (0,0,128), (0,0,139), (0,0,205), 
        #       (0,0,255), (65,105,225), (152,251,152), (144,238,144),
        #       (50,205,50), (255,255,0), (255,215,0), (255,165,0), 
        #       (255,140,0), (255,99,71), (255,0,0)]

        
        self.image_data = image.reshape(240,320)
        gauss_filter = gaussian_filter(self.image_data, sigma=1)
        self.sensor_image.setImage(gauss_filter)       
        # cmap = pg.ColorMap(pos=np.linspace(0.0, 1.0, 15), color=colors)
        # self.sensor_image.setColorMap(cmap)
