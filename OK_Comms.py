...
class OK_Comms():
    ...
    def ok_header_func(self, OKbitfile):
        ...
        error = self.dev.ConfigureFPGA(OKbitfile)
    ...
    def trigger(self, bank, ProgResetDACName):
        ...
        self.dev.ActivateTriggerIn(address, int(bit))
    ...
    def wireoutdata(self, bank, wirename):
        ...
        address = bank['addr'].iloc[bankindex]
        address = int(address,16)
        readvalue = self.dev.GetWireOutValue(address)
        bs = bank['bit'].iloc[bankindex]
        data = readvalue<<bs
    ...
    def wireindata(self, bank, wirename, data):
        ...
        bankindex = list(name).index(wirename)
        banksize = bank['size'].iloc[bankindex]
        bit = bank['bit'].iloc[bankindex]
        address = bank['addr'].iloc[bankindex]
        address = int(address,16)
    ...
    def readfromblockpipeout(self, epaddr, blksize, bsize):
        ...
        buf = bytearray(psize)
        self.dev.ReadFromBlockPipeOut(epaddr, blksize, buf)
